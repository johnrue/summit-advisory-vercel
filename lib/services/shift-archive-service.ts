    ...
    try {
      ...
      if (query.sortBy) {
        ...
        switch (query.sortBy) {
          ...
          case 'revenue':
            ...
            break;
          case 'satisfaction':
            baseQuery = baseQuery.order('client_satisfaction_score', { ascending, nullsFirst: false });
            break;
          case 'duration':
      ...
      return {
        ...
        data: {
          ...
          facets: {
            clients: Object.entries(clientCounts).map(([name, count]) => ({ name, count: count as number })),
  ...
  static async getArchiveMetrics(
    ...
    try {
      ...
      const archivesByMonthArray = Object.entries(archivesByMonth)
        .map(([month, data]) => ({ month, count: (data as any).count, revenue: (data as any).revenue }))
